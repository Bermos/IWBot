...
public class BGS {
	...
	public static void logActivity(Activity activity, String userid, String username, int ammount) {
		Connection connect = new Connections().getConnection();
		try {
		...
		Statistics.getInstance().logBGSActivity(System.currentTimeMillis(), userid, username, activity.toString(), ammount);
	}
